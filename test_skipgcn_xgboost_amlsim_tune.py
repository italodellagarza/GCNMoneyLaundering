...
#
# test_skipgcn_xgboost_amlsim_tune.py: SkipGCN + XGBoost
...
def training_function(config):
    ...
    for _ in range(5):
        ...
        loss = torch.nn.CrossEntropyLoss(
            weight=torch.Tensor(
                [1.0 - config['weight_1'], config['weight_1']]
            )
        )
    ...
    session.report(
        {"loss": (val_loss / val_steps), "f10": f1_0},
...
def train_test_best(best_results):
    ...
    model = SkipGCN(8, best_results.config['neurons_l1'], 2)
    # Create the optimizer
    optimizer = torch.optim.Adam(
        model.parameters(),
        lr=best_results.config['learning_rate']
    )
    # Create loss
    loss = torch.nn.CrossEntropyLoss(
        weight=torch.Tensor(
            [
                1.0 - best_results.config['weight_1'],
                best_results.config['weight_1']
            ]
        )
    )
    ...
    checkpoint_path = os.path.join(
        best_results.checkpoint.to_directory(),
        'checkpoint.pt'
    )
