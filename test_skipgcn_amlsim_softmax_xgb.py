...
#
# test_skipgcn_amlsim_softmax_xgb.py: Tests for SkipGCN + Softmax 
#                                     + XGBoost
#
...
def main():
    ...
    for execution in range(n_repeats):
        ...
        # Model definition
        model = SkipGCN(8, 22, 2)
        model = model.to('cpu')

        loss = torch.nn.CrossEntropyLoss(
            weight=torch.Tensor([1 - 0.73601, 0.73601])
        )

        optimizer = torch.optim.Adam(model.parameters(), lr=0.006703)
        ...
        if len(embeddings_test[y_pred == 1.0]) == 0:
            continue
